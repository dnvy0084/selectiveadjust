...
이미지의 밝기 변경은 간단합니다. 각 픽셀에 동일한 값을 더하거나 빼는걸로 이미지의 밝기를 조절할 수 있습니다. 
...
대비는 어두운 색은 더 어둡게, 밝은 색은 더 밝게 조절하여 이미지를 좀 더 뚜렷하게 보이도록 만들어줍니다. 
...
0에서 255까지 분포되어 있는 값에 128을 빼서 -128 ~ 128로 변경하고 곱하기를 하면 음수는 더 작게 양수는 더 크게 바뀌게 됩니다. 그 후 128을 더해 정상 범위로 돌려놓으면 됩니다. 

### 채도

채도는 높을수록 이미지의 색이 선명해지고 낮을수록 무채색에 가깝게 변합니다. 우선 이미지를 무채색으로 만들려면 R, G, B 세가지 값의 평균을 구하면 되는데요, 1 / 3씩 균등하게 주면 안되고 각 색마다 비율이 정해져 있습니다. 순서대로 (0.2125, 0.7154, 0.0721)인데 우리눈의 시세포 중 색을 인식하는 원추세포의 분포에 따른 비율이라고 합니다. 

```javascript
r = 0.2125 * r + 0.7154 * g + 0.0721 * b;
g = 0.2125 * r + 0.7154 * g + 0.0721 * b;
b = 0.2125 * r + 0.7154 * g + 0.0721 * b;
```

saturation이 0일때 흑백, 1일 때 원래 색이 나와야 되니 아래와 같이 수정할 수 있습니다. 

```javascript
const s = saturation;

r = s * r + (1 - s) * 0.2125 * r + (1 - s) * 0.7154 * g + (1 - s) * 0.0721 * b;
g = s * g + (1 - s) * 0.2125 * r + (1 - s) * 0.7154 * g + (1 - s) * 0.0721 * b;
b = s * b + (1 - s) * 0.2125 * r + (1 - s) * 0.7154 * g + (1 - s) * 0.0721 * b;
```
